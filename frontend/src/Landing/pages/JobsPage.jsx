...
export default function JobBoard() {
  ...
  const jobsToShowMobile = showAllMobile ? dummyJobs : dummyJobs.slice(0, 5);
  ...
  return (
    <div className="min-h-screen bg-blue-20 px-4 py-12">
      ...
      <div className="sm:hidden grid grid-cols-1 gap-6">
        ...
        {dummyJobs.length > 5 && (
          <button
            ...
            className="mx-auto mt-6 px-5 py-2 bg-blue-600 text-white rounded-full hover:bg-blue-700 transition-all duration-300"
          >
      ...
      {/* Desktop CTA Button */}
      <div className="text-center mt-14 hidden sm:block">
        <motion.button
...
const JobCard = ({ job, hoveredJob, setHoveredJob, navigate }) => (
  ...
  >
    ...
    <div className="group relative bg-white border border-gray-200 rounded-2xl shadow-lg p-6 flex flex-col justify-between transition-all duration-300 ease-in-out hover:shadow-xl overflow-hidden">
      {job.urgent && (
        <div className="absolute top-4 right-4 bg-orange-500 text-white px-3 py-1 rounded-full text-xs font-bold flex items-center gap-1 z-10 shadow-lg">
          🔥 Urgent
        </div>
      )}
